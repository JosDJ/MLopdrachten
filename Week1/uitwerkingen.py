...
def draw_graph(data):
    ...
    # Omdat je de x en y moet scheiden van elkaar in de matrix, je wilt twee vectoren krijgen
    # 1 met de x waarden de ander met de y waarden
    ...
    x, y = data.T
    plt.scatter(x,y)
    plt.xlabel("Populatie in 10k personen")
    plt.ylabel("Winst in 10k dollar")
    plt.show()
...
def compute_cost(X, y, theta):
    ...
    J = 0
    m = y.shape[0] # lengte bepalen
    p = np.dot(X, theta) # voorspelling X keer theta
    errors = (p - y)**2 # fout berekenen
    J = np.sum(errors)/(2*m) #alles optellen en delen door aantal punten * 2
...
def gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for i in range(1,num_iters):
        ...
        theta[0][0] = theta[0][0] - alpha * g[0]
        theta[0][1] = theta[0][1] - alpha * g[1]


        #np.dot(theta,alpha*g[0])

        costs.append(compute_cost(X,y,theta.T))

    # aan het eind van deze loop retourneren we de nieuwe waarde van theta
    # (wat is de dimensionaliteit van theta op dit moment?). (1,2)
