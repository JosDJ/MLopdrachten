...
def build_model():
    ...
    # dimensionaliteit hiervan?). (28,28)
...
def conf_els(conf, labels): 
    ...
    for i in range (len(labels)):
        tpi = tp[i]
        fpi = np.sum(conf[:,i])-tpi
        fni = np.sum(conf[i,0:])-tpi
        tni = np.sum(np.sum(conf))-(tpi+fpi+fni)  #10000 - (tpi+fpi+fni)
        namen = ['categorie','tp','fp','fn','tn']
        waarde = [labels[i],tpi, fpi,fni,tni]
        result = zip(namen, waarde)
        lijst.append(list(result))
    return lijst
...
def conf_data(metrics):
    ...
    tp = [sum(m[1][1] for m in metrics)][0]
    fp = [sum(m[2][1] for m in metrics)][0]
    fn = [sum(m[3][1] for m in metrics)][0]
    tn = [sum(m[4][1] for m in metrics)][0]
    ...
    # tpr geeft de verhouding tussen tussen de goed geclassifiseerde objecten en het totaal van dat object
    # dus hoe hoger hoe beter

    #ppv is de verhouding tussen goed geclassifiseerde objecten /
    # en het totaal aantal objecten dat die classificatie heeft gekregen./
    # hoe hoger hoe beter

    # tnr geeft de verhouding tussen het totaal het totaal correct 'negatief' geclassificeerde objecten/
    # en het totaal negatief geclassificeerde obj

    # fpr geeft de verhouding tussen het aantal verkeerd negatief geclassificeerde objecten en het totaal
    # negatief geclassificeerde objecten.
